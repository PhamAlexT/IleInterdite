...
class VueCommandes extends JPanel implements Observer {
	...
	/**
	 * VueCommandes() : Ile modele : une ile. DeroulementPartie dp : une partie.
	...
	public VueCommandes(Ile modele, DeroulementPartie dp) {
		...
		recupArte.addActionListener(e -> {
			if (liaison.getAJActuel().estLibre()) {
				dp.getAJActuel().getrA().donnerArtefact();
				;
			}
		});
		...
		echangeArte.addActionListener(e -> {
			if (liaison.getAJActuel().estLibre()) {
				dp.getAJActuel().getEchangeClefs().echange();
			}
		});
	...
	/**
	 * void setGrille() : VueGrille vg : une VueGrille. Donne la valeur de la
	 * VueGrille vg a l'attribut vg de la VueCommandes.
	 **/
